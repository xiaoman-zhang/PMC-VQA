...
from model.QA_model_mlp import QA_model
...
class ModelArguments:
    model_path: Optional[str] = field(default="/mnt/petrelfs/wuchaoyi/chatGPT_APIs/PMC_QA_final/PMC_QA_project/LLAMA/llama-7b-hf")
    ckp: Optional[str] = field(default="/mnt/petrelfs/wuchaoyi/chatGPT_APIs/PMC_QA_final/PMC_QA_project/Results/QA_PMC_LLaMA_lora_PMC-CLIP_MLP/choice_training/checkpoint-4146")
    ...
    visual_model_path: Optional[str] = field(default='/mnt/petrelfs/wuchaoyi/chatGPT_APIs/PMC_QA_final/PMC_QA_project/img_checkpoint/PMC-CLIP/checkpoint.pt')
...
@dataclass
class DataArguments:
    img_dir: str = field(default='/mnt/petrelfs/share_data/wuchaoyi/PMC_Images/medical_compound_figrues/', metadata={"help": "Path to the training data."})
    Test_csv_path: str = field(default='/mnt/petrelfs/wuchaoyi/chatGPT_APIs/PMC_QA_final/PMC_QA_project/Data/final_train/final_test.csv', metadata={"help": "Path to the training data."})
    tokenizer_path: str = field(default='/mnt/petrelfs/wuchaoyi/chatGPT_APIs/PMC_QA_final/PMC_QA_project/LLAMA/tokenizer', metadata={"help": "Path to the training data."})
...
def main():
    ...
    with open('result_final_'+model_args.ckp.split('/')[-3]+'_'+ model_args.ckp.split('/')[-2]+'.csv', mode='w') as outfile:
            ...
            for sample in tqdm.tqdm(Test_dataloader):
                ...
                #     break 
    # if os.path.exists('result_final'+str(data_args.trier)+'.csv'): 
    #     with open('result_final'+str(data_args.trier)+'.csv', mode='a') as outfile:
    #         writer = csv.writer(outfile)
    #         # writer.writerow(['Figure_path','Pred','Label','Correct'])
    ...
    #             #     continue
    #             # i=i+1
    #             input_ids = Test_dataset.tokenizer(sample['input_ids'],return_tensors="pt",padding='longest').to('cuda')
    #             images = sample['images'].to('cuda')
    #             with torch.no_grad():
    #                 generation_ids = model.generate(input_ids['input_ids'],images)
    #             generated_texts = Test_dataset.tokenizer.batch_decode(generation_ids.argmax(-1), skip_special_tokens=True) 
    ...
    #                     ACC = ACC +1
    #                     corret = 1 
    #                 writer.writerow([img_path,sample['input_ids'][0]+generated_texts[i][-1],label,corret])
    #                 cc = cc + 1             
    print(ACC/cc)  
